...
var JSON_KEYS = {};
JSON_KEYS.system_json = ['name', 'tiebreaks', 'playervotes', 'faction_name', 'slug'];
...
dbfun.initialiseTournamentTables = function initialiseTournamentTables(tObject, callback) {
  ...
  playerQuery += 'playervotes int[] DEFAULT \'{}\',';
...
dbfun.updatePlayerDetails = function updatePlayerDetails(updateObject, callback) {
  ...
  dbfun.checkTournament(updateObject.tKey, function(exists, actualKey) {
    if (exists) {
      ...
      } else if (typeof updateObject.index === 'number') {
        field += '[' + updateObject.index + ']';
      };
      ...
      if (field.match(/(score)|(tiebreak)|(playervotes)/g)) {
