...
function addPlayerRow(customID,tableID) {
  ...
  var tableRowContent = '<tr id="t' + tableID + 'playerRow' + newID + '">' +
    ...
    '<td style="text-align: center;"><input type="number" size="3" id="t' + tableID + 'score' + newID + '" /></td>';
  for ( var i = 0; i < tiebreaks.length; i++ ) {
    tableRowContent += '<td style="text-align: center;"><input type="number" size="3" id="t' + tableID + 'tiebreak' + i + newID + '" /></td>';
  };
  ...
  $( '#t' + tableID + 'score' + newID ).bind('focusout change', function() {
    ...
    socket.emit( 'scoresChanged', updateObject );
    return false;
  });
  
  for ( var i = 0; i < tiebreaks.length; i++ ) {
    $( '#t' + tableID + 'tiebreak' + i + newID ).bind('focusout change', function() {
      var updateObject = {tKey:common.tournamentKey, id:newID, round:tableID};
      updateObject.field = 'tiebreak';
      updateObject.value = [];
      updateObject.value[i] = $(this).val();
      socket.emit( 'scoresChanged', updateObject );
      return false;
    });
  };
  
  $( '#faction' + newID ).bind( 'focusout', function() {
    var updateObject = {tKey:common.tournamentKey, id:newID, round:tableID};
